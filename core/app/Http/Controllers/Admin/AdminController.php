...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        // User Info
        $widget['total_users']             = User::where('user_type', 'User')->count();
        $widget['verified_users']          = User::where('user_type', 'User')->active()->count();
        $widget['email_unverified_users']  = User::where('user_type', 'User')->emailUnverified()->count();
        $widget['mobile_unverified_users'] = User::where('user_type', 'User')->mobileUnverified()->count();

        // Field Agent Info
        $widget['total_field_agents']             = User::where('user_type', 'Agent')->count();
        $widget['active_field_agents']            = User::where('user_type', 'Agent')->activeagent()->count();
        $widget['banned_field_agents']  = User::where('user_type', 'Agent')->banned()->count();
        ...
        // Add this section to fetch pending loans
        $pendingLoans = Loan::pending()
                            ->with('user:id,username,account_number', 'plan')
                            ->latest()
                            ->take(10)
                            ->get();

        return view('admin.dashboard', compact('pageTitle', 'widget', 'chart', 'deposit', 'withdrawals', 'pendingLoans'));
    }
