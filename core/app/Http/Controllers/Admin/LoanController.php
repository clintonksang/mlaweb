...
class LoanController extends Controller
{
    ...
    public function exportPdf($scope = null, $userId = 0)
    {
        $query = Loan::orderBy('id', 'DESC');
        if ($scope) {
            $query->$scope();
        ...
        $pageTitle = $this->pageTitle;
        $loans = $query->searchable(['loan_number', 'user:username'])
            ->dateFilter()
            ->filter(['status'])
            ->with('user:id,username', 'plan', 'plan.category', 'nextInstallment')
            ->get();

        $pdf = Pdf::loadView('admin.loan.pdf', compact('loans', 'pageTitle'));
