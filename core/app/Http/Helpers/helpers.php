...
function getTemplates()
{
    ...
    $param['website'] = @$_SERVER['HTTP_HOST'] . @$_SERVER['HTTP_HOST'] . @$_SERVER['REQUEST_URI'] . ' - ' . env("APP_URL");
...
function insertCountriesFromJson() {
    $filePath = resource_path('views/partials/country.json');
    if (!file_exists($filePath)) {
        throw new Exception("Country JSON file not found.");
    }

    $countries = json_decode(file_get_contents($filePath), true);
    if (!$countries) {
        throw new Exception("Invalid JSON format in country file.");
    }
